...
void State8086::interrupt(unsigned int vector) {
   ...
   push(segRegs.CS);
   push(IP);
   segRegs.CS = mem->getmem<uint16_t>((vector << 2) + 2);
   IP = mem->getmem<uint16_t>(vector << 2);
   alu.flags.I = 0; // reset interrupt flag
   alu.flags.T = 0; // reset trap flag
}

template<>
uint8_t& State8086::fetchreg(unsigned int regid) {
   assert(regid < 8);
   switch (regid) {
...
template<>
uint16_t& State8086::fetchreg(unsigned int regid) {
   assert(regid < 8);
   switch (regid) {
...
void State8086::ModRegRm::operator()() {
   uint8_t addrbyte = state->imm<uint8_t>(); // read ModR/M byte
   mode = (addrbyte >> 6) & 0b111;
   reg = (addrbyte >> 3) & 0b111;
   rm = (addrbyte >> 0) & 0b111;
   ...
   {
      if (rm == 6)
         state->disp16 = state->imm<uint16_t>();
      if (((rm == 2) || (rm == 3)) && !state->segoverride)
         state->segment = state->segRegs.SS;
      break;
   }
   case 1: // 8 bit displacement follows ModR/M byte to be sign-extended and added to the index (unkown source) (p 2-4?)
   {
      state->disp16 = (int16_t)state->imm<uint8_t>(); // Sign extend immediate byte to 16 bit
      if (!state->segoverride && ((rm == 2) || (rm == 3) || (rm == 6)))
         state->segment = state->segRegs.SS;
      break;
   }
   case 2: // 16 bit displacement follows ModR/m byte to be added to the index
   {
      state->disp16 = state->imm<uint16_t>();
      if (!state->segoverride && ((rm == 2) || (rm == 3) || (rm == 6)))
         state->segment = state->segRegs.SS;
...
void State8086::push(uint16_t value) {
   mem->putmem<uint16_t>(segRegs.SS, pi_regs.SP -= 2, value);
}

uint16_t State8086::pop() {
   return mem->getmem<uint16_t>(segRegs.SS, (pi_regs.SP += 2) - 2);
}

int State8086::getea() {
   int tempea = 0;
   switch (modregrm.mode) {
   case 0:
      switch (modregrm.rm) {
...
void State8086::run(unsigned int runtime)
{
   ...
   for (unsigned int count = 0; count < runtime;) {
      ...
      case 0xB0: // MOV AL             move imm8 to r8 (IA V2 p316)
      case 0xB1: // MOV CL             move imm8 to r8 (IA V2 p316)
      case 0xB2: // MOV DL             move imm8 to r8 (IA V2 p316)
      case 0xB3: // MOV BL             move imm8 to r8 (IA V2 p316)
      case 0xB4: // MOV AH             move imm8 to r8 (IA V2 p316)
      case 0xB5: // MOV CH             move imm8 to r8 (IA V2 p316)
      case 0xB6: // MOV DH             move imm8 to r8 (IA V2 p316)
      case 0xB7: // MOV BH             move imm8 to r8 (IA V2 p316)
      {
         fetchreg<uint8_t>(
      ...
      case 0xB8: // MOV AX             move imm16 to r16 (IA V2 p316)
      case 0xB9: // MOV CX             move imm16 to r16 (IA V2 p316)
      case 0xBA: // MOV DX             move imm16 to r16 (IA V2 p316)
      case 0xBB: // MOV BX             move imm16 to r16 (IA V2 p316)
      case 0xBC: // MOV SP             move imm16 to r16 (IA V2 p316)
      case 0xBD: // MOV BP             move imm16 to r16 (IA V2 p316)
      case 0xBE: // MOV SI             move imm16 to r16 (IA V2 p316)
      case 0xBF: // MOV DI             move imm16 to r16 (IA V2 p316)
      {
         fetchreg<uint16_t>(
            opcode & 0b0111 // reg in lower 3 bits
            ) = imm<uint16_t>();
      ...
      case 0xA0: // MOV AL,moffs8      move byte at (seg:offset) to AL (IA V2 p316)
      {
         d_regs.a.l = mem->getmem<uint8_t>(segment, imm<uint16_t>());
         break;
      }
      case 0xA1: // MOV AX,moffs16     move word at (seg:offset) to AX (IA V2 p316)
      {
      ...
      case 0xA2: // MOV moffs8,AL      move AL to (seg:offset) (IA V2 p316)
      {
         mem->putmem<uint8_t>(segment, imm<uint16_t>(), d_regs.a.l);
         break;
      }
      case 0xA3: // MOV moffs16,AX     move AX to (seg:offset) (IA V2 p316)
      {
      ...
      // Register
      // [01010 reg]
      case 0x50: // PUSH AX            push AX (IA V2 page 415)
      case 0x51: // PUSH CX            push CX (IA V2 page 415)
      case 0x52: // PUSH DX            push DX (IA V2 page 415)
      case 0x53: // PUSH BX            push BX (IA V2 page 415)
      case 0x54: // PUSH SP            push SP (IA V2 page 415)
      case 0x55: // PUSH BP            push BP (IA V2 page 415)
      case 0x56: // PUSH SI            push SI (IA V2 page 415)
      case 0x57: // PUSH DI            push DI (IA V2 page 415)
      {
      ...
      // Segment register
      // [000 reg 110]
      case 0x06: // PUSH ES            push ES (IA V2 p415)
      case 0x0E: // PUSH CS            push CS (IA V2 p415)
      case 0x16: // PUSH SS            push SS (IA V2 p415)
      case 0x1E: // PUSH DS            push DS (IA V2 p415)
      {
         push(segRegs[
      ...
      {
         modregrm(); // fetch [mod reg r/m] byte
         assert(modregrm.reg == 0); // Only support POP operation (IA V2 p380)
         writerm<uint16_t>(pop());
         break;
      }
      // Register
      // [01011 reg]
      case 0x58: // POP AX             pop top of stack into AX; increment stack pointer (IA V2 p380)
      case 0x59: // POP CX             pop top of stack into CX; increment stack pointer (IA V2 p380)
      case 0x5A: // POP DX             pop top of stack into DX; increment stack pointer (IA V2 p380)
      case 0x5B: // POP BX             pop top of stack into BX; increment stack pointer (IA V2 p380)
      case 0x5C: // POP SP             pop top of stack into SP; increment stack pointer (IA V2 p380)
      case 0x5D: // POP BP             pop top of stack into BP; increment stack pointer (IA V2 p380)
      case 0x5E: // POP SI             pop top of stack into SI; increment stack pointer (IA V2 p380)
      case 0x5F: // POP DI             pop top of stack into DI; increment stack pointer (IA V2 p380)
      {
      ...
      // Segment register
      // [000 reg 111]
      case 0x07: // POP ES             pop top of stack into ES; increment stack pointer (IA V2 p380)
      ...
      case 0x17: // POP SS             pop top of stack into SS; increment stack pointer (IA V2 p380)
      case 0x1F: // POP DS             pop top of stack into DS; increment stack pointer (IA V2 p380)
      {
      ...
      // [10010 reg]
      case 0x90: // NOP                no operation (IA V2 p340)
      {
         // Alias for XCHG AX,AX. This has no effect
         break;
      }
      case 0x91: // XCHG CS            exchange CX with AX (IA V2 p492)
      case 0x92: // XCHG DX            exchange DX with AX (IA V2 p492)
      case 0x93: // XCHG BS            exchange BX with AX (IA V2 p492)
      case 0x94: // XCHG SP            exchange SP with AX (IA V2 p492)
      case 0x95: // XCHG BP            exchange BP with AX (IA V2 p492)
      case 0x96: // XCHG SI            exchange SI with AX (IA V2 p492)
      case 0x97: // XCHG DI            exchange DI with AX (IA V2 p492)
      {
      ...
      // Register
      // [01000 reg]
      case 0x40: // INC AX             increment AX by 1 (IA V2 p243)
      case 0x41: // INC CX             increment CX by 1 (IA V2 p243)
      case 0x42: // INC DX             increment DX by 1 (IA V2 p243)
      case 0x43: // INC BX             increment BX by 1 (IA V2 p243)
      case 0x44: // INC SP             increment SP by 1 (IA V2 p243)
      case 0x45: // INC BP             increment BP by 1 (IA V2 p243)
      case 0x46: // INC SI             increment SI by 1 (IA V2 p243)
      case 0x47: // INC DI             increment DI by 1 (IA V2 p243)
      {
      ...
      // Register
      // [01001 reg]
      case 0x48: // DEC AX             decrement AX by 1 (IA V2 p112)
      case 0x49: // DEC CX             decrement CX by 1 (IA V2 p112)
      case 0x4A: // DEC DX             decrement DX by 1 (IA V2 p112)
      case 0x4B: // DEC BX             decrement BX by 1 (IA V2 p112)
      case 0x4C: // DEC SP             decrement SP by 1 (IA V2 p112)
      case 0x4D: // DEC BP             decrement BP by 1 (IA V2 p112)
      case 0x4E: // DEC SI             decrement SI by 1 (IA V2 p112)
      case 0x4F: // DEC DI             decrement DI by 1 (IA V2 p112)
      {
